...
# library straight from commandbar.def, so no version of LibMaker is
...
# ordinal (ordinals are pinned in commandbar.def and never change).
#
...
# =====================================================================
param(
    [string]$DefFile = "$PSScriptRoot\commandbar.def",
    [string]$DllName = "commandbar.dll",
    [string]$OutFile = "$PSScriptRoot\..\clarion\commandbar.lib"
)
