...
| A slider, spin, progress or gallery draws as a plain button with its text on it | an old `commandbar.dll`. The generic `CB_AddItem` used to reject any type above `CBI_SPACE` and quietly hand back a button, so generated code was fine and the picture was wrong. Fixed by the DLL alone — nothing to regenerate |
